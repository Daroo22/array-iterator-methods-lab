...
for (let i = 0; i < inventors.length; i++) {
  ...
  if (inventor.hasOwnProperty('first') && inventor.hasOwnProperty('last')) {
    inventorNames.push({ first: inventor.first, last: inventor.last });
  } else {     console.log(`Skipping inventor with missing properties: ${JSON.stringify(inventor)}`);
}
}
...
let sortedByBirthYear = inventors.slice();

// Enter your solution code here:

for (let i = 0; i < sortedByBirthYear.length - 1; i++) {
  for (let j = 0; j < sortedByBirthYear.length - 1 - i; j++) {
    if (sortedByBirthYear[j].year > sortedByBirthYear[j + 1].year) {
      let temp = sortedByBirthYear[j];
      sortedByBirthYear[j] = sortedByBirthYear[j + 1];
      sortedByBirthYear[j + 1] = temp;
    }
  }
}
...
for (let i = 0; i < inventors.length; i++) {
  ...
  if (inventor.first === 'Ada') {
    ...
    break; // Exit the loop once 'Ada' is found
  }
}
...
for (let i = 0; i < people.length; i++) {
  let name = people[i];
  let splitName = name.split(', '); // Split the name into an array [last, first]
  if (splitName.length === 2) {
    let formattedName = `${splitName[1]} ${splitName[0]}`; // Rearrange to "First Last" format
    firstLast.push(formattedName);
  } else {    console.log(`Skipping invalid name format: ${name}`);
}
}
...
let isEveryone19OrOlder = true; 

for (let i = 0; i < devs.length; i++) {
  let currentYear = new Date().getFullYear(); 
  let age = currentYear - devs[i].year; 

  if (age < 19) {
    isEveryone19OrOlder = false; 
    break; 
  }
}
...
for (let i = 0; i < inventors.length; i++) {
  let lifespan = inventors[i].passed - inventors[i].year;
...
for (let i = 0; i < travelMethods.length; i++) {
  let method = travelMethods[i];
  
  if (travelMethodCounts[method]) {
    travelMethodCounts[method]++; 
  } else {
    travelMethodCounts[method] = 1; 
  }
}
